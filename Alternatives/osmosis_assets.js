...
// RPC endpoints
osmosis_rpc = "https://lcd.osmosis.zone/";
...
async function fetchBalanceData() {
  try {
    const response = await axios.get(`${osmosis_rpc}${all_balance_api}`);
...
async function fetchDelegationRewards() {
  try {
    const response = await axios.get(`${osmosis_rpc}${delegation_rewards_api}`);
    return response.data;
  } catch (error) {
